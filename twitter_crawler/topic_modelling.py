...
def topic_of_tweet(text):
    ...
    '''
    tokenized_text = word_tokenizer.tokenize(re.sub("(@[A-Za-z0-9]+)|([^0-9A-Za-z \t])|(\w+:\/\/\S+)", " ", text))
    #print(tokenized_text)
    lemmatized_words = []
    for word in tokenized_text:
        lemmatized_words.append(lemmatize(word.lower()))
